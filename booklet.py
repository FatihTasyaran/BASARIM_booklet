...
def txt2tex(file_name, verbose=False) :
    ...
    if(lang == "TUR"):
        ...
        if(link.find("contact") != -1):
            link_final = "\\newline\\newline\\noindent \\bfseries{Bu bildiriye erişim için yazarlarla iletişime geçiniz}"

        else:
            link_final = r""" \newline\newline\noindent \href{}""".format('{' + link +'}{\\bfseries Çevrimiçi Erişim}')
    ...
    with open(tex_dir + file_type + 'b_' + file_id + '.tex', "w") as text_file:
        text_file.write("\phantomsection\n")
